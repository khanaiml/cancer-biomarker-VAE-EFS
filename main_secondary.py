...
sys.path.append('.')
from config.config_secondary import SECONDARY_CONFIG
...
def main():
    ...
    print("="*80)
    print("BIOMARKER IDENTIFICATION PIPELINE - SECONDARY DATASET")
    print("="*80)
    ...
    plt.ylabel('Accuracy (%)')
    plt.title('Model Performance vs Number of Genes (Secondary Dataset)')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('results/secondary/performance_curve.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    anova_results.to_csv('results/secondary/anova_results.csv', index=False)
    ...
    corr_results.to_csv('results/secondary/correlation_results.csv', index=False)
    ...
    pd.DataFrame([results_summary]).to_json(
        'results/secondary/summary.json', orient='records', indent=2
    )
    
    pd.DataFrame({'biomarker': top_biomarkers}).to_csv(
        'results/secondary/biomarkers.txt', index=False, header=False
    )
    ...
    print("\nResults saved in: results/secondary/")

if __name__ == "__main__":
    os.makedirs('results/secondary', exist_ok=True)
    main()